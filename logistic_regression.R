...
mu <- 0    # mean for data samples
sigma <- 1   #sd for data samples

X <- matrix(1,n,d) # matrix to hold data (including column of 1s)

for(i in 1:n){
  X[i,-1] <-rnorm(q,mu,sigma)
}

### Data matrices for each model
...
betas <- c(1,2,-1.5) # beta values chosen to generate our response
p <- 1/(1+exp(-(X5.1%*%betas))) # p values of beta values using TRUE model

y <- numeric(n) # y is a vector which holds our response from the TRUE model
for( i in 1:n){
  y[i] <- rbinom(1,m,p[i])
}
...
priorsigma <- 100 # standard deviation of Gaussian prior for betas
sets <- 50 # number of chains of posterior and prior samples
iterations <-11000 # number of iterations from each chain
warmup <- 1000 # number of iterations used as warmup for each chain
N <- iterations - warmup # size of our posterior and prior samples after warm up has been discarded
...
for(i in 1:sets){
  mean <- apply(chains[[i]],2,mean)
  var <- var(chains[[i]])
  BSproposalsims[[i]] <- rmvnorm(N2,mean = mean, sigma = var)
}

BSproposalpvals <- list()

for(i in 1:sets){
  BSproposalpvals[[i]] <- t(apply(BSproposalsims[[i]],1,function(x) {1/(1+exp(-(X1.1%*%x)))}))
}

likelihoods2 <- matrix(0,sets,N2)

for ( i in 1:sets) {
  likelihoods2[i,] <- apply(BSproposalpvals[[i]],1,likelihood, y = y, n = m)
}

priors2 <- matrix(0,sets,N2)
for ( i in 1:sets) {
  priors2[i,] <- apply(BSproposalsims[[i]],1,prior, priormu = priormu, priorsigma = priorsigma)
}
...
for(i in 1:sets){
  mean <- apply(chains[[i]],2,mean)
  var <- var(chains[[i]])
  proposaldensities2[i,] <- apply(BSproposalsims[[i]],1,dmvnorm, mean = mean, sigma = var)
}
...
FIdensityestimates <- apply(matrix,1,function(x){1/(N*pi^d)*sum(x)})

centrepvals <- t(apply(centres,1,function(x) {1/(1+exp(-(X1.1%*%x)))}))

FIlikelihoods <- apply(centrepvals,1,likelihood,y = y,n = m)

FIpriors <- numeric(sets)

for(i in 1:sets){
  FIpriors[i] <- prior(centres[i,],priormu = priormu, priorsigma = priorsigma)
}

FIestimates1 <- log(FIlikelihoods*FIpriors)-log(FIdensityestimates)

FIestimate1 <- mean(FIestimates1)
FIsd1 <- sqrt(var(FIestimates1))
...
for(i in 1:sets){
  BSproposalpvals[[i]] <- t(apply(BSproposalsims[[i]],1,function(x) {1/(1+exp(-(X2.1%*%x)))}))
}
...
centrepvals <- t(apply(centres,1,function(x) {1/(1+exp(-(X2.1%*%x)))}))
...
FIestimates2 <- log(FIlikelihoods*FIpriors)-log(FIdensityestimates)

FIestimate2 <- mean(FIestimates2)
FIsd2 <- sqrt(var(FIestimates2))
...
for(i in 1:sets){
  BSproposalpvals[[i]] <- t(apply(BSproposalsims[[i]],1,function(x) {1/(1+exp(-(X3.1%*%x)))}))
}
...
centrepvals <- t(apply(centres,1,function(x) {1/(1+exp(-(X3.1%*%x)))}))
...
FIestimates3 <- log(FIlikelihoods*FIpriors)-log(FIdensityestimates)

FIestimate3 <- mean(FIestimates3)
FIsd3 <- sqrt(var(FIestimates3))
...
for(i in 1:sets){
  BSproposalpvals[[i]] <- t(apply(BSproposalsims[[i]],1,function(x) {1/(1+exp(-(X4.1%*%x)))}))
}
...
centrepvals <- t(apply(centres,1,function(x) {1/(1+exp(-(X4.1%*%x)))}))
...
FIestimates4 <- log(FIlikelihoods*FIpriors)-log(FIdensityestimates)

FIestimate4 <- mean(FIestimates4)
FIsd4 <- sqrt(var(FIestimates4))
...
for(i in 1:sets){
  BSproposalpvals[[i]] <- t(apply(BSproposalsims[[i]],1,function(x) {1/(1+exp(-(X5.1%*%x)))}))
}
...
centrepvals <- t(apply(centres,1,function(x) {1/(1+exp(-(X5.1%*%x)))}))
...
FIestimates5 <- log(FIlikelihoods*FIpriors)-log(FIdensityestimates)

FIestimate5 <- mean(FIestimates5)
FIsd5 <- sqrt(var(FIestimates5))
...
for(i in 1:sets){
  BSproposalpvals[[i]] <- t(apply(BSproposalsims[[i]],1,function(x) {1/(1+exp(-(X6.1%*%x)))}))
}
...
centrepvals <- t(apply(centres,1,function(x) {1/(1+exp(-(X6.1%*%x)))}))
...
FIestimates6 <- log(FIlikelihoods*FIpriors)-log(FIdensityestimates)

FIestimate6 <- mean(FIestimates6)
FIsd6 <- sqrt(var(FIestimates6))
...
for(i in 1:sets){
  BSproposalpvals[[i]] <- t(apply(BSproposalsims[[i]],1,function(x) {1/(1+exp(-(X7.1%*%x)))}))
}
...
centrepvals <- t(apply(centres,1,function(x) {1/(1+exp(-(X7.1%*%x)))}))
...
FIestimates7 <- log(FIlikelihoods*FIpriors)-log(FIdensityestimates)

FIestimate7 <- mean(FIestimates7)
FIsd7 <- sqrt(var(FIestimates7))
...
df <- data.frame(
  ...
  'FI' = c(FIestimate1,FIsd1,FIestimate2,FIsd2,FIestimate3,FIsd3,FIestimate4,FIsd4,FIestimate5,FIsd5,FIestimate6,FIsd6,FIestimate7,FIsd7)
)
...
density_plot4 <- mcmc_areas(samples[,1:3,],  regex_pars = "beta0",
)
...
plot <- mcmc_areas_ridges(samples[,1,], regex_pars = "beta", border_size = 0.75)

plot + xlim(c(-2,3)) + theme(plot.margin = margin(t = 0, r = 0, b = 3, l = 3))
